...
export default function DoctorLayout() {
  const { user } = useAuth();
  console.log('doctor layout', user);
  const location = useLocation();
  const { searchQuery, setSearchQuery } = useSearch();
  const allowedManageAdminRoles = ['DOCTOR'];
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="h-16 bg-[#154D92] px-6 flex justify-between items-center">
          <div className={`flex items-center ${location.pathname.includes('/doctor/dashboard') ? 'invisible' : 'visible'}`}>
